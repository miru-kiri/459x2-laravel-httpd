    ...
    <div class="my-3">
      ...
      <div class="table-responsive">
        <table class="table table table-bordered">
          ...
          @php
              // Lấy thời gian hiện tại (dạng HHMM)
              $now = \Carbon\Carbon::now();
              $currentWeekday = $now->dayOfWeek; // 0 (Chủ nhật) - 6 (Thứ 7)
              $currentTimeInt = (int) $now->format('Hi'); // VD: 17:30 -> 1730

              $isNowBreakTime = false;

              if (!empty($siteBreaks[$currentWeekday])) {
                  foreach ($siteBreaks[$currentWeekday] as $breakTime) {
                      $start = (int) str_replace(':', '', $breakTime['start']);
                      $end = (int) str_replace(':', '', $breakTime['end']);
                      ...
                      @else
                          @if($result['result'] == 1)
                              <td class="text-center">
                                  <button class="btn btn-block btn-possible" data-date_time="{{ $result['dateTime'] }}">
                                      <i class="far fa-circle"></i>
                                  </button>
                              </td>
                          @elseif($result['result'] == 2)
                              <td class="text-center">
                                  <button class="btn btn-block tel-btn">TEL</button>
                              </td>
                          @else
                              <td class="text-center">-</td>
                          @endif
