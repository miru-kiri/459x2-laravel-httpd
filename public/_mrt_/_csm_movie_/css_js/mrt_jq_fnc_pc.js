...
(function($) {
   $.mrt_1text_swinger = function(){
      ...
      if($(oya_obj_nm).size()){
         //var all_obj = $(box_obj_nm).html();
         //$(txt_obj_nm).css({"width": $(txt_obj_nm).eq(0).width() + "px"});
         //$(oya_obj_nm).remove();
         ...
         var sw_obj_w = $("body").width();
         var sw_obj_h = $("body").height();
         var txt_obj_w = $(txt_obj_nm).eq(0).width();
         if(txt_obj_w < 100){
            txt_obj_w = 1000;
         }
         $(oya_obj_nm).css({"position":"relative", "overflow":"hidden", "white-space":"nowrap"});
         $(txt_obj_nm).css({"display":"block", "position":"absolute", "top":"0px", "left": sw_obj_w + "px", "white-space":"nowrap"});
         ...
         var ani1_w  = sw_obj_w / 3;
         var ani2_w  = (txt_obj_w / 10) * 7;
         var ani_end = txt_obj_w;
         ...
         ani(ani1_w, ani2_w, ani_end, sw_obj_w);
         //<--setinterval
         var sintID = setInterval(function(){
            var sani1_w   = ani1_w;
            var sani2_w   = ani2_w;
            var sani_end  = ani_end;
            $(".mrteen_mes").html(txt_obj_w);
            var ssw_obj_w = sw_obj_w;
            ani(sani1_w, sani2_w, sani_end, ssw_obj_w);
         }, 6300); //setinterval-->
         
         
         //animate
         function ani(xani1_w, xani2_w, xani_end, xsw_obj_w){
            //var txt_obj_w = $(txt_obj_nm).eq(0).width();
            //var sw_obj_w = $("body").width();
            $(txt_obj_nm).eq(0).animate({
               top: 0,
               left: xani1_w
            },{
               'duration': 200,
               ...
               'easing': 'linear',
               'complete': function(){
                  $(txt_obj_nm).css({"top":"0px", "left": xsw_obj_w + "px"});
               }
            });
         }
